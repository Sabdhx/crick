...
export const login = async (req, res, next) => {
  ...
  try {
      ...
      if (!user) {
          return next(customError(401, "User not found or incorrect email"));
      }
      ...
      switch (user.role) {
          ...
          case "player":
              try {
                  ...
                  return next(customError(500, "Error creating player data"));
              }
              return res.status(200).json({ userName: user.username, email: user.email, role: user.role, token });
          ...
          case "hiringManager":
              try {
                  ...
                  return next(customError(500, "Error creating Hiring Manager data"));
              }
              return res.status(200).json({ userName: user.username, email: user.email, role: user.role, token });
              
          default:
              return next(customError(400, "Invalid user role"));
      }
